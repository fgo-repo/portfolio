<template>   
    <div>
        <label class="rocker rocker-small">
            <input type="checkbox" :value="getTheme" :checked="getTheme == 'light'" @input="setTheme($event.target.value)">
...
<script>
  export default {
    ...
    mounted() {
        const cookieValue = this.$cookies.get('THEME')

        if(cookieValue !== undefined){
            if(cookieValue !== this.$store.state.theme){
                this.setTheme(cookieValue === 'light' ? 'dark' : 'light')
            }            
        }
        else{
